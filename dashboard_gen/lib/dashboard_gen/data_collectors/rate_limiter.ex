defmodule DashboardGen.DataCollectors.RateLimiter do
  ...
  defp reset_service_counters(state, service, period) do
    ...
    %{state | counters: new_counters, last_reset: new_last_reset}
  end
  ...
  defp reset_all_counters(state, period) do
    ...
    new_counters =
      Enum.reduce(services, state.counters, fn service, acc ->
        put_in(acc, [period, service], 0)
      end)

    new_last_reset =
      Enum.reduce(services, state.last_reset, fn service, acc ->
  ...
  defp calculate_reset_times do
    ...
    %{
      "15min" => DateTime.add(now, 15 * 60, :second) |> DateTime.truncate(:second),
      minute: DateTime.add(now, 60, :second) |> DateTime.truncate(:second),
      hour: DateTime.add(now, 3600, :second) |> DateTime.truncate(:second),
      day: DateTime.add(now, 24 * 3600, :second) |> DateTime.truncate(:second)
    }
  end
