defmodule DashboardGen.DataCollectors.StatusMonitor do
  ...
  def handle_info(:check_status, state) do
    ...
    new_history =
      [%{timestamp: DateTime.utc_now(), status: new_status} | state.status_history]
      |> Enum.take(100)
  ...
  def handle_call(:get_detailed_status, _from, state) do
    detailed_status = %{
      collectors: state.collectors_status,
      alerts: state.alerts,
      # Last 20 status checks
      history: Enum.take(state.status_history, 20),
  ...
  defp check_for_dead_collectors(status, alerts) do
    dead_collectors =
      Enum.filter(status, fn {_type, collector_status} ->
  ...
  defp check_for_stale_collections(status, alerts) do
    ...
    stale_collectors =
      Enum.filter(status, fn {type, collector_status} ->
  ...
  defp calculate_overall_health(collectors_status) do
    ...
    if total_collectors == 0 do
      ...
      healthy_collectors =
        Enum.count(collectors_status, fn {_type, status} ->
